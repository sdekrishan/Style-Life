...
export const getdata =
  ...
  async (dispatch) => {
    ...
    if (sort == "pop") {
      try {
        let res = await axios.get(
          `https://busy-cyan-betta-garb.cyclic.app/${cat}/filter?page=${page}&&popular=true&&limit=6`
        );
      ...
      try {
        let res = await axios.get(
          `https://busy-cyan-betta-garb.cyclic.app/${cat}/filter?page=${page}&&sort=${val}&limit=6`
        );
        dispatch({ type: GET_PRODUCT, payload: res.data });
      } catch (err) {
        console.log(err);
    ...
    // try {
    //   let res = await axios.get(
    //     `https://busy-cyan-betta-garb.cyclic.app/${cat}?_page=${page}&_limit=12&_sort=${sort}&_order=${order}`
    //   );
...
export const filterdata = (val, page, sort, order, cat) => async (dispatch) => {
  ...
  if (sort == "pop") {
    ...
    try {
      res = await axios.get(
        `https://busy-cyan-betta-garb.cyclic.app/${cat}/filter?page=${page}&&sort=${v}&limit=6`
      );
    } catch (err) {
      console.log(err);
    }
